...
## Вещи которые пришлось сделать

- Закоммитить production базу в репозиторий, так как приложение сделано для конкурса, и если запускать на локальной машине то в приложении нет статей
- ActiveStorage Blobs пришлось закоммитить для того чтобы в Render не платить за volume и картинки статей отображались
